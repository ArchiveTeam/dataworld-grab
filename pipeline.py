...
LUA_SHA1 = get_hash(os.path.join(CWD, 'dataworld.lua'))
    ...
    def realize(self, item):
        wget_args = [
            ...
            '--domains', ','.join([
                ...
                'download.data.world',
                'media.data.world',
                'mediauploads.data.world',
                'mediauploads-us.data.world',
                'query.data.world',
                'cdn.filepicker.io',
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'search':
                wget_args.extend(['--warc-header', 'dataworld-search: '+item_value])
                wget_args.append('https://data.world/api/search?type=dataset&size=100&from=0&filters=created%3D'+item_value)
            elif item_type == 'project':
                wget_args.extend(['--warc-header', 'dataworld-project: '+item_value])
                wget_args.append('https://data.world/'+item_value)
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'dataworld-asset: '+url])
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/5/5d/Data.world-icon.png" height="50px" title=""/>
        <h2>data.world <span class="links"><a href="https://data.world/">Website</a> &middot; <a href="https://tracker.archiveteam.org/dataworld/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Data.world">Wiki</a></span></h2>
        <p>Archiving public data.world.</p>
    ''',
    utc_deadline=datetime.datetime(2026, 7, 14, 0, 0, 0)
)
